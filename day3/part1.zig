...
pub fn main() !void {
    ...
    for (1..140) |x| {
        for (1..140) |y| {
            if (std.ascii.isDigit(data[y][x]) or data[y][x] == '.') continue;
            sum += try getAdjacentNumberSum(data, x, y);
        }
    }
...
fn getAdjacentNumberSum(data: anytype, x: usize, y: usize) !u64 {
    var sum: u64 = 0;
    ...
    while (y_index <= @min(y + 1, data.len - 1)) : (y_index += 1) {
        ...
        while (x_index <= @min(x + 1, data[y_index].len - 1)) : (x_index += 1) {
            ...
            sum += try std.fmt.parseInt(u64, line[start .. end + 1], 10);
            x_index = end;
        }
    }
    return sum;
...
test "Get adjacent number sum" {
    ...
    const sumx3y1 = try getAdjacentNumberSum(data, 3, 1);
    try std.testing.expectEqual(@as(u64, 10000 + 5 + 9 + 22), sumx3y1);

    const sumx3y2 = try getAdjacentNumberSum(data, 3, 2);
    try std.testing.expectEqual(@as(u64, 22 + 9 + 5), sumx3y2);
}
